...
void power_gpio_init(void);          /* configure 45/42/3, install GPIO3→42 ISR */
void power_log_battery(void);        /* one-shot dump of PMIC charge/battery state */
void power_log_boot_reason(void);    /* log esp_reset_reason + wake cause at boot */
void power_pre_deep_sleep(void);     /* DC4 off, GPIO45 driven high & held for sleep */
